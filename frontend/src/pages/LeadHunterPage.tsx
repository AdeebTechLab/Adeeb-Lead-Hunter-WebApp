import { AlertTriangle, Cloud, ExternalLink, Mail, MapPin, Phone, Search, Sparkles } from 'lucide-react'
...
type ProviderResponse = { default: string; providers: ProviderOption[] }
...
export default function LeadHunterPage() {
  ...
  const [meta, setMeta] = useState<Omit<SearchResponse, 'items' | 'count'> | null>(null)
  ...
  async function search(event: FormEvent) {
    ...
    try {
      ...
      toast.success(`${result.items.length} leads found`)
    } catch (error) {
      setItems([])
    ...
    <div className="stack-lg">
      <section className="card hunter-card">
        ...
        <form className="hunter-form" onSubmit={search}>
          ...
          <label>City<div className="input-with-icon plain"><MapPin size={17} /><input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} required /></div></label>
        ...
        <div className="source-note"><Cloud size={15} />Searches configured business-data APIs with automatic fallback and contact enrichment.</div>
                ...
                <tr key={`${lead.business_name}-${lead.source_url || index}`}>
                  ...
                  <td>
                    <div className="contact-cell">
                      <strong>{lead.phone || lead.email || 'No direct contact published'}</strong>
                      <span>{lead.contact_status || 'Research needed'} · {lead.contact_confidence || 'Low'} confidence</span>
                      <div className="contact-actions">
                        {lead.phone && <a href={`tel:${lead.phone}`} title="Call"><Phone size={14} /></a>}
                        {lead.email && <a href={`mailto:${lead.email}`} title="Email"><Mail size={14} /></a>}
                        {lead.contact_search_url && <a href={lead.contact_search_url} target="_blank" rel="noreferrer" title="Review Google Maps listing"><ExternalLink size={14} /></a>}
                      </div>
                    </div>
                  </td>
